...
if __name__ == '__main__':
    ...
    if( args.use_datagen ):
        evals = model.evaluate_generator( datagen_train, steps = args.n_samplings, workers = args.n_workers, use_multiprocessing = True, verbose = 1 )
    else:
        evals = model.evaluate( x = X_train, y = y_train, batch_size = args.batch_size, verbose = 1 )
    ...
    # 予想ラベルを推論
    if( args.use_datagen ):
        predicts = model.predict_generator( datagen_test, steps = args.n_samplings, workers = args.n_workers, use_multiprocessing = True, verbose = 1 )    
    else:
        predicts = model.predict( X_test, batch_size = args.batch_size, verbose = 1 ) 
