...
def load_dataset(
    dataset_dir, 
    image_height = 32, image_width = 32, n_channels =3, n_classes = 10,
    one_hot_encode = True,
):
    ...
    X_train_resize = np.zeros( shape=(len(X_train), image_height, image_width, n_channels) )
    for i in range(len(X_train)):
        X_train_cv = cv2.resize(X_train[i], dsize = (image_width,image_height), interpolation = cv2.INTER_AREA)
        for c in range(n_channels): 
            X_train_resize[i,:,:,c] = X_train_cv
    
    X_test_resize = np.zeros( shape=(len(X_test), image_height, image_width, n_channels) )
    for i in range(len(X_test)):
        X_test_cv = cv2.resize(X_test[i], dsize = (image_width,image_height), interpolation = cv2.INTER_AREA) 
        for c in range(n_channels): 
            X_test_resize[i,:,:,c] = X_test_cv
