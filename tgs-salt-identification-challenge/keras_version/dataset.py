...
def load_dataset(
    ...
    for i, name in enumerate(image_names):
        img = cv2.imread( os.path.join( dataset_dir, "train", "images", name + ".png" ) )
        img = cv2.resize( img, (image_height, image_width), interpolation = cv2.INTER_LANCZOS4 )  # shape = [H,W,C]
        X_train[i] = img

    # y_train（マスク画像）
    y_train = np.zeros( (len(image_names), image_height, image_width, 1), dtype=np.uint8 )
    for i, name in enumerate(image_names):
        img = cv2.imread( os.path.join( dataset_dir, "train", "masks", name + ".png" ), cv2.IMREAD_GRAYSCALE )
    ...
    # X_test
    image_names = sorted( [f for f in os.listdir(os.path.join( dataset_dir, "test", "images")) if f.endswith(IMG_EXTENSIONS)] )
    image_names = image_names[0: min(n_samplings, len(image_names))]
